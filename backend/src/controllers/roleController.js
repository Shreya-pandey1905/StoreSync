...
const QUERY_OPTIONS = { maxTimeMS: 8000 };

// Get the logged-in user's role (lightweight — used by permission context)
  ...
  try {
    ...
    if (!role) {
      return res.status(404).json({
        success: false,
        message: `Role "${req.user.role}" not found. Run npm run seed in the backend folder.`
      });
    }
    ...
    res.status(500).json({
      success: false,
      message: 'Error fetching your role',
      error: error.message
    });
  }
};
...
const getAllRoles = async (req, res) => {
  try {
    ...
    // Get total count
    const totalRoles = await Role.countDocuments(filter).maxTimeMS(QUERY_OPTIONS.maxTimeMS);
