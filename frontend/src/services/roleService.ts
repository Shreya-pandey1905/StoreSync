...
// Get the logged-in user's role (faster than loading every role)
export const getMyRole = async (): Promise<ApiResponse<Role>> => {
  try {
    const response = await axiosInstance.get('/roles/me');
    return response.data;
  } catch (error: any) {
    console.error('Error fetching current user role:', error);
    throw new Error(error.response?.data?.message || 'Failed to fetch your role');
  }
};
