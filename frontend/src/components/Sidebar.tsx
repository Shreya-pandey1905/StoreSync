...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onToggle }) => {
  ...
  return (
    <aside
      className={`fixed left-0 top-0 h-full bg-white dark:bg-slate-900 border-r border-slate-100 dark:border-slate-800 transition-all duration-300 z-50 shadow-sm
        ${isOpen ? 'translate-x-0 w-64' : '-translate-x-full lg:translate-x-0 lg:w-20'}
      `}
    >
      <div className="flex items-center justify-between p-3 sm:p-4 border-b border-slate-100 dark:border-slate-800">
        {isOpen && (
        ...
        {navigation.map((item) => {
          ...
          return (
            <Link
              ...
              className={`flex items-center gap-3 px-3 py-2.5 sm:py-3 rounded-xl transition-all ${isOpen ? '' : 'lg:justify-center'
                } ${active
                  ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 border-l-[3px] border-blue-600 dark:border-blue-400 font-semibold shadow-sm'
      ...
      <div className="absolute bottom-0 left-0 right-0 p-2 sm:p-3 border-t border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900">
